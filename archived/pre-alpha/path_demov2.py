...
import pygame, json, math, time, os, glob
...
def find_latest_waypoint_json():
    ...
    if not cands:
        return None
    # prefer exact 'waypoint.json' if it exists and is latest; otherwise newest by mtime
    return max(cands, key=lambda p: os.path.getmtime(p))

def load_json(path=None):
    """Loads last path + meta; updates CENTER, cm/px, speed/height, mode, and path arrays."""
    global points, grid_path, mode, CM_PER_PX, FLIGHT_SPEED, FLIGHT_HEIGHT, CENTER, info_msg
    try:
        if path is None:
            path = find_latest_waypoint_json() or "waypoint.json"
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        pos  = data.get("pos", [])
        meta = data.get("meta", {})
        if not pos:
            info_msg = f"No 'pos' in {os.path.basename(path)}"
            return
        ...
        # update map/flight settings (if provided)
        ...
        if m == "GRID":
            ...
            mode = MODE_FREE
            # pos[0] is CENTER; remaining are user points
            if len(pos) >= 1:
                points = [(int(x),int(y)) for x,y in pos[1:]]
            else:
                points = []
            grid_path.clear()

        info_msg = f"Loaded {os.path.basename(path)} | mode={m} | points={len(pos)}"
        add_blink(CENTER)
    except FileNotFoundError:
        info_msg = "No waypoint.json found."
